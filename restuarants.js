...
restuarants =[
  ...
  const displayOperatingHours = (id) => {
    const restaurant = restuarants.find(rest => rest.id === id);
    if (restaurant) {
  ...
  displayOperatingHours(4);
  

  const displayStephComments = (restaurantName, customerName) => {
    const restaurant = restuarants.find(rest => rest.name === restaurantName);
    if (restaurant) {
      const review = restaurant.reviews.find(rev => rev.name === customerName);
      if (review) {
        console.log(review.comments);
      } else {
        console.log(`${customerName} has not provided a review for ${restaurantName}`);
      }
    } else {
      console.log(`${restaurantName} restaurant not found`);
    }
  };
  
  displayStephComments("Emily", "Steph");
  

  const displayAsianRestaurants = () => {
    const asianRestaurants = restuarants.filter(rest => rest.cuisine_type === "Asian");
    asianRestaurants.forEach(rest => {
      console.log(rest.name);
    });
  };
  
  displayAsianRestaurants();
  
  const displayBrooklynRestaurants = () => {
    const brooklynRestaurants = restuarants.filter(rest => rest.neighborhood === "Brooklyn");
    brooklynRestaurants.forEach(rest => {
      console.log(rest.name); 
    });
  };
  
  displayBrooklynRestaurants();
